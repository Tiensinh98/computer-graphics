...
glm::vec3 eyepos(0, -2, 2);  // initial eye position
glm::vec3 center(0.0, 0.0, 0.0);  // initial center that the eye look at
glm::vec3 up(0, 1, 1); // initial up vector
...
glm::mat3 rotate(const float degrees, const glm::vec3& axis) {
	double radians = double(degrees * 3.1416 / 180);
	...
	glm::mat3 unchanged_cmp(cos(radians), 0, 0, 0, cos(radians), 0, 0, 0, cos(radians));
	glm::mat3 along_cmp(one_minus_cos * x_square, one_minus_cos * xy, one_minus_cos * xz, one_minus_cos * xy, one_minus_cos * y_square, one_minus_cos * yz, one_minus_cos * xz, one_minus_cos * yz, one_minus_cos * z_square);
	glm::mat3 rotate_cmp(0, -sin(radians) * axis.z, sin(radians) * axis.y, sin(radians) * axis.z, 0, -sin(radians) * axis.x, -sin(radians) * axis.y, sin(radians) * axis.x, 0);
	glm::mat3 rotation_matrix = unchanged_cmp + along_cmp + rotate_cmp;
...
void mousedrag(int x, int y) {
	/* NOTES:
	1. eye position is the input of glm::lookAt, not the eye vector
	*/

	if (mouseButton == GLUT_MIDDLE_BUTTON)
	{
		/* Rotate around the lookat center*/
		// moved distance of mouse in pixels
		int delta_x = x - mouseoldx;
		int delta_y = mouseoldy - y;  // due to the positive y-axis in screen is downward with the origin is top-left corner of the screen
		if (abs(delta_x) + abs(delta_y) == 0) return;

		// 
		up = glm::normalize(up);
		glm::vec3 eye_vec = glm::normalize(eyepos - center);
		glm::vec3 right = glm::normalize(glm::cross(up, eye_vec)); // re-calculate right vector

		// move-vector unit of mouse in the world space
		glm::vec3 m_vec = -glm::normalize(right * float(delta_x) + up * float(delta_y));
		glm::vec3 axis = glm::cross(eye_vec, m_vec);
		float angle = (sqrt(pow(delta_x, 2) + pow(delta_y, 2)) / windowWidth) * 360.0;
		eye_vec = rotate(angle, axis) * eye_vec;
		up = rotate(angle, axis) * up;
		eyepos = center + eye_vec * glm::length(eyepos - center);
		mouseoldy = y, mouseoldx = x;
	}
	else if (mouseButton == GLUT_RIGHT_BUTTON)
	{
		/* Translate the lookat center */
		int delta_x = x - mouseoldx;
		int delta_y = mouseoldy - y;    // We will use the y coord to shift the origin
		glm::vec3 eye_vec = glm::normalize(eyepos - center);
		glm::vec3 right = glm::normalize(glm::cross(up, eye_vec)); // re-calculate right vector
		glm::vec3 m_vec = -0.005f * (right * float(delta_x) + up * float(delta_y));
		center += m_vec;
		mouseoldy = y, mouseoldx = x;
	}

	/* Re-calculate the view matrix */
	modelview = glm::lookAt(eyepos, center, up);
...
void updateCenterScaling(int x, int y, float amount)
{
	glm::vec3 m_vec = glm::normalize(glm::vec3(1.0, 0.0, 0.0) * float(x - windowWidth / 2) + glm::vec3(0.0, 1.0, 0.0) * float(windowHeight / 2 - y));
	printf("(%.2f, %.2f, %.2f\n)", m_vec.x, m_vec.y, m_vec.z);
	glm::vec3 old_center = center;
	center = old_center + m_vec * amount;
	eyepos = center + (eyepos - old_center) * (1.0f + amount);
	modelview = glm::lookAt(eyepos, center, up);
	// Send the updated matrix to the shader
	glUniformMatrix4fv(modelviewPos, 1, GL_FALSE, &(modelview)[0][0]);
	glutPostRedisplay();
...
void keyboard(unsigned char key, int x, int y)
{
	...
	case 's': // ** NEW ** to turn on/off shading (always smooth) ; 
		...
		break;
	case '+':
		updateCenterScaling(x, y, scaleAmount);
		break;
	case '-':
		updateCenterScaling(x, y, -scaleAmount);
		break;
