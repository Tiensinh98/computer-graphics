...
void display()
{
	...
	if (numused) {
		...
		{
			GLfloat templightposn[4];
			templightposn[i % 4] = lightposn[i];
			if ((i + 1) % 4 == 0)
			{
				GLfloat outtemplightposn[4];
				transformvec(templightposn, outtemplightposn);
				for (int j = int(i / 4) * 4; j < int(i / 4) * 4 + 4; j++)
				{
					lightransf[j] = outtemplightposn[j % 4];
				}
			}
		}
		glUniform1i(numusedcol, numused);
		glUniform4fv(lightpos, numLights, &lightransf[0]); // numLigts = 10 tell that in the shader we have a array of 10 vec4
